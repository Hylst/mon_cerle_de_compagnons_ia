...
// Fix: Switched from extending EventEmitter to composition to resolve type errors.
export class AudioRecorder {
  ...
  async start() {
    ...
    const promise = startTask()
      .catch(err => {
        // Stop and clean up resources if starting fails
        this.stop();
        // Re-throw the error to be handled by the caller
        throw err;
      })
      .finally(() => {
        // Once start is complete (or has failed), clear the starting promise
        this.starting = null;
      });
  ...
  stop() {
    ...
    if (this.starting) {
      // If start() is in progress, wait for it to finish before stopping
