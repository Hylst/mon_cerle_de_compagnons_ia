...
export class GenAILiveClient {
  ...
  public async connect(config: LiveConnectConfig): Promise<boolean> {
    ...
    try {
      ...
      this.emitter.emit('error', new ErrorEvent('error', { message }));
      return false;
    }
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      ...
      this.emitter.emit(
        'error',
        new ErrorEvent('error', { message: 'Le client n\'est pas connecté' })
      );
      return;
    }
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      if (serverContent.modelTurn) {
        ...
        const otherParts = _.difference(parts, audioParts);
