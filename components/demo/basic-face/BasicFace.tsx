...
export default function BasicFace({
  ...
  const [scale, setScale] = useState(isEditing ? 1 : 0.1);
  ...
  useEffect(() => {
    // For the editing preview, we use a fixed size based on the radius prop
    // and don't need to dynamically scale based on the window size.
    if (isEditing) {
      setScale(1);
      return;
    }
